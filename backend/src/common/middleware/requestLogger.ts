...
const consoleLogger = pino({
  level: "info",
  transport: {
    targets: [{ target: "pino-pretty", options: { colorize: true }, level: "info" }],
  },
});
...
if (env.FILE_LOGGING_ENABLED) {
  ...
  const fileLogger = pino({
    level: "info",
    transport: {
      targets: [{ target: "pino/file", options: { destination: logFile }, level: "info" }],
    },
  });
  multiLogger = {
    fatal: (arg) => {
      fileLogger.fatal(arg);
      consoleLogger.fatal(arg);
    },
    error: (arg) => {
      fileLogger.error(arg);
      consoleLogger.error(arg);
    },
    warn: (arg) => {
      fileLogger.warn(arg);
      consoleLogger.warn(arg);
    },
    info: (arg) => {
      fileLogger.info(arg);
      consoleLogger.info(arg);
    },
    debug: (arg) => {
      fileLogger.debug(arg);
      consoleLogger.debug(arg);
    },
    trace: (arg) => {
      fileLogger.trace(arg);
      consoleLogger.trace(arg);
    },
  };
  requestFileLogger = pinoHttp({
    ...
    customErrorMessage: (_req: Request, res: Response) => `Request failed with status code: ${res.statusCode}`,
    serializers: {
      req: (req: Request) => ({
        method: req.method,
        url: req.url,
